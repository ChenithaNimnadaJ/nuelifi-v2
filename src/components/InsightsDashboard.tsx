...
export function Insights({ meals, actions, analytics, subscription, preferences, onUpdatePreferences, dataState, dataError, onUpgrade, onOpenMeal, showFreeAds: _showFreeAds }: InsightsProps) {
  ...
  const chartRows = useMemo(() => rows.map((row, index) => { const windowStart = Math.max(0, index - 6); const windowRows = rows.slice(windowStart, index + 1); const serverPoint = serverChart.get(row.capturedAt); return { id: row.id, label: serverPoint?.label && /^[A-Za-z]{3,9}\\s+\\d{1,2}$/.test(serverPoint.label) ? serverPoint.label : row.label, fullDate: formatDate(row.capturedAt, timezone), score: serverPoint?.score ?? row.score, rollingAverage: serverPoint?.rollingAverage ?? Math.round((windowRows.reduce((total, item) => total + item.score, 0) / windowRows.length) * 10) / 10, period: row.period }; }), [rows, serverChart, timezone]);
  ...
  const timingCompared = timing.filter((item) => item.count >= 2 && item.average !== null);
  ...
  return <div className="insights-page"><div className="insights-page-heading"><div><span className="eyebrow">INSIGHTS</span><h1>Make your meal history useful.</h1><p>Clear patterns from the meals you actually save. No invented conclusions.</p></div><div className="insights-data-status">{dataState === "loading" ? "Loading your data…" : dataState === "error" ? dataError || "Some data is unavailable." : `${rows.length} meal${rows.length === 1 ? "" : "s"} in your history`}</div></div>
    <section className="insights-card insights-chart-card"><div className="insights-card-heading"><div><span className="insights-kicker">MEAL BALANCE TREND</span><h2>Your meal scores over time</h2><p>Every saved meal is shown. The dashed line is your rolling seven-meal average.</p></div><span className="insights-target-chip">Target {numberLabel(analytics?.chart.target.min ?? 70)}–{numberLabel(analytics?.chart.target.max ?? 100)}</span></div>{chartRows.length ? <div className="insights-chart" aria-label="Meal balance trend chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartRows} margin={{ top: 12, right: 12, left: -14, bottom: 4 }}><ReferenceArea y1={analytics?.chart.target.min ?? 70} y2={analytics?.chart.target.max ?? 100} fill="var(--chart-target)" fillOpacity={0.12} /><CartesianGrid stroke="var(--border)" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="label" axisLine={false} tickLine={false} interval={chartRows.length > 8 ? Math.ceil(chartRows.length / 7) - 1 : 0} tick={{ fill: "var(--muted)", fontSize: 11 }}/><YAxis domain={[0, 100]} axisLine={false} tickLine={false} ticks={[0, 50, 70, 100]} tick={{ fill: "var(--muted)", fontSize: 11 }}/><Tooltip content={<TrendTooltip/>}/><Line type="monotone" dataKey="score" stroke="var(--chart-line)" strokeWidth={2.5} dot={{ r: 3, fill: "var(--chart-line)" }} activeDot={{ r: 5 }} name="Meal balance"/><Line type="monotone" dataKey="rollingAverage" stroke="var(--brand)" strokeWidth={2} strokeDasharray="6 5" dot={false} name="Rolling average"/></LineChart></ResponsiveContainer></div> : <div className="insights-empty"><strong>No meal trend yet</strong><span>Save a few meals and your score history will appear here.</span></div>}<div className="insights-chart-legend"><span><i className="legend-score"/>Meal balance</span><span><i className="legend-average"/>7-meal average</span><span><i className="legend-target"/>Target zone</span></div></section>
    <div className="insights-grid insights-premium-grid">{isPremium ? <section className="insights-card"><div className="insights-card-heading"><div><span className="insights-kicker">PREMIUM · 1</span><h2>Meal timing patterns</h2><p>When do you tend to eat better? This compares your own saved meals, not a universal rule.</p></div></div>{timingCompared.length >= 2 ? <><div className="timing-list">{timing.map((item) => <div className={`timing-row ${strongest?.period === item.period ? "timing-best" : ""}`} key={item.period}><span>{item.period}</span><div className="timing-track"><i style={{ width: `${item.average === null ? 0 : Math.max(4, Math.min(100, item.average))}%` }}/></div><strong>{numberLabel(item.average, "/100")}</strong><small>{item.count} meal{item.count === 1 ? "" : "s"}</small></div>)}</div><p className="insights-supporting-copy">Your strongest period is <strong>{strongest?.period.toLowerCase()}</strong>. This is an observed pattern, not proof that timing causes the difference.</p></> : <div className="insights-empty"><strong>Not enough data yet</strong><span>Keep logging meals across at least two time periods to compare your pattern.</span></div>}</section> : <LockedFeature tier="premium" title="Meal timing patterns" question="When do I tend to eat better?" copy="Compare your own meal scores by time of day when enough history exists." onUpgrade={() => onUpgrade("analytics")}/>} 
      {isPremium ? <section className="insights-card"><span className="insights-kicker">PREMIUM · 2</span><h2>My focus</h2><p>One clear area to work on, based on where your own saved meals are weakest.</p>{weakest && strongest && focusDelta && focusDelta >= 5 ? <><div className="focus-callout"><span>FOCUS</span><strong>{weakest.period} meals</strong><p>{numberLabel(weakest.average, "/100")} average compared with {numberLabel(strongest.average, "/100")} in the {strongest.period.toLowerCase()}.</p></div><p className="insights-supporting-copy">Review the suggestions from your recent {weakest.period.toLowerCase()} meals and look for one small change to repeat.</p></> : <div className="insights-empty"><strong>Not enough contrast yet</strong><span>Keep logging meals so Neulifi can identify one useful focus area from your own history.</span></div>}</section> : <LockedFeature tier="premium" title="My focus" question="What should I work on?" copy="See one clear, data-backed focus area instead of a wall of statistics." onUpgrade={() => onUpgrade("analytics")}/>} 
