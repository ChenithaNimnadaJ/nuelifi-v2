...
export interface ResultAction { id: string; title: string; detail: string; completed: boolean; added: boolean; existing?: boolean; }
...
export function MealResults({ meal, actions, onAction, onAddTask, onDone, showFreeAds }: { meal: Meal; actions: ResultAction[]; onAction: (id: string) => void; onAddTask: (id: string) => void; onDone: () => void; showFreeAds: boolean }) {
  ...
  const actionable = actions.filter((action) => !action.existing);
  return <div className="screen results-screen"><div className="result-hero"><img src={meal.imageUrl} alt={meal.mealName}/><div className="result-hero-copy"><p className="eyebrow light">MEAL BALANCE</p><Badge>{meal.analysis.rating}</Badge><div className="result-score">{meal.analysis.score}<small>/100</small></div></div></div><h1 className="result-title">Here is what Neulifi noticed</h1><p className="result-summary">{meal.analysis.explanation}</p><section className="result-section"><h3>Approximate signals from your photo</h3><div className="nutrition-grid">{values.map((item) => <div className="nutrition-item" key={item.label}><small>{item.label}</small><strong>{item.value}</strong></div>)}</div></section><section className="result-section"><h3>What is looking good</h3><div className="positive-list">{good.map((item) => <p key={item}><span>{icon("check", 14)}</span>{item}</p>)}</div></section><section className="result-section"><h3>One or two ways to improve</h3><div className="improve-list">{improve.slice(0, 2).map((item) => <p key={item}>• {item}</p>)}</div></section><section className="result-section recommendations"><h3>How could you improve this meal?</h3>{meal.analysis.mealGuidance.length ? meal.analysis.mealGuidance.map((item, index) => <div className="recommendation" key={`${item}-${index}`}><span>{index + 1}</span><div><strong>{item}</strong><small>Meal-specific guidance — not added to tasks.</small></div></div>) : <p className="empty-task-note">No meal-specific change is needed for this meal.</p>}</section><section className="result-section result-actions"><div className="result-actions-heading"><h3>{actionable.length ? "Your next best step" : actions.length ? "Already on your plan" : "No next step needed"}</h3></div>{actions.length ? actions.map((action, index) => <div className="result-action-row" key={action.id}>{action.existing ? <div className="result-action result-action-existing"><span className="result-checkbox">✓</span><span className="result-action-copy"><strong>Already on your plan</strong><small>{action.detail}</small></span></div> : <><button type="button" className={`result-action ${action.completed ? "completed" : ""} ${!action.added ? "pending-task" : ""}`} disabled={!action.added} onClick={() => onAction(action.id)}><span className="result-checkbox">{action.completed ? "✓" : ""}</span><span className="result-action-copy">{actionable.length > 1 && action === actionable[1] && <small className="result-action-label">Optional second step</small>}<strong>{action.title}</strong><small>{action.detail}</small></span></button>{action.added ? <span className="result-action-added">Added</span> : <button type="button" className="add-task-button" onClick={() => onAddTask(action.id)}>+ Add to Tasks</button>}</>}</div>) : <p className="empty-task-note">Nothing specific is needed right now. Continue normally.</p>}</section><div className="bottom-guidance">{actions.length ? <><strong>You don't need to change everything.</strong><span>Start with one improvement.</span></> : <><strong>Nothing to fix right now.</strong><span>Continue normally and scan again when useful.</span></>}</div>{showFreeAds && <FreeAds slot="analysis"/>}<button type="button" className="button button-green done-button" onClick={onDone}>Done — back to dashboard</button></div>;
}
