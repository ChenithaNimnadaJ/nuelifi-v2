...
async function paddleClientToken(env) { const environment = paddleEnvironment(env); const response = await paddleFetch(env, "/client-tokens?status=active&per_page=200"); if (!response.ok) throw new Error("Paddle client-side token could not be loaded."); const payload = await response.json().catch(() => ({})); const prefix = environment === "sandbox" ? "test_" : "live_"; const token = (Array.isArray(payload.data) ? payload.data : []).map((item) => typeof item === "object" && item ? String(item.token || "").trim() : "").find((value) => new RegExp(`^${prefix}[A-Za-z0-9]{27}$`).test(value)); if (!token) throw new Error("Paddle client-side token is not configured."); return token; }
