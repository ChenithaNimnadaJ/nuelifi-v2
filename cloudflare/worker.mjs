...
function taskFamily(value) { const text = String(value || ""); if (/walk|movement|move|break/i.test(text)) return "movement"; if (/water|hydrate/i.test(text)) return "hydration"; if (/sleep|bedtime|rest|wind down/i.test(text)) return "rest"; if (/planning|schedule|routine|priority|goal/i.test(text)) return "planning"; return ""; }
function completeDailyTasks(items, context = {}, mealName = "", indicators = {}) { const selected = cleanList(items, 4).filter(isSafeDailyTask); const existing = Array.isArray(context?.existingActions) ? context.existingActions.map((item) => String(item || "").trim()).filter(Boolean) : []; return [...new Set(selected)].filter((task) => { const family = taskFamily(task); return !family || !existing.some((item) => taskFamily(item) === family); }).slice(0, 2); }
...
function mealPrompt(mealName, context = {}, analysisLevel = "basic") { return `You are Neulifi, a calm food and lifestyle companion. The output is machine-read and persisted by the application. Analyse the meal called "${mealName}" from this one photo. ${contextSummary(context)} ${analysisDepth(analysisLevel)} Treat this as user-provided context only: never infer a diagnosis, prescribe treatment, recommend medication changes, or overrule a clinician. Avoid known allergens and respect dietary preferences. Nutrition values are estimates. Return only JSON. Keep mealGuidance and dailyTasks strictly separate: mealGuidance is about this photographed meal or a future similar meal and may mention ingredients, portions, nutrients, substitutions, or preparation; dailyTasks are at most two small actions for the person’s day and may be connected to this meal when genuinely useful, such as a short walk after this meal, but must not tell them to edit, add to, remove from, swap, portion, inspect, label-check, or otherwise change the photographed meal or a specific ingredient. ${localTaskContext(context)} For dailyTasks only, return 0 to 2 short, broad actions for the person’s day. Keep them useful but non-specific: hydration, a movement break, rest or wind-down, planning, or a simple routine. If no action is genuinely warranted, return an empty dailyTasks array. Do not invent generic tasks for a balanced meal, and do not repeat an existing open action. Never suggest eating, cooking, changing a meal, a specific food, ingredient, nutrient, portion, recipe, or meal time. Never use an exact clock time, named exercise other than a short walk, medical action, medication instruction, diagnosis, or condition-specific treatment. If it is overnight, prefer rest, hydration, or planning for later rather than suggesting food or movement now. Use cautious, non-judgmental language. Return rating, score, indicators, explanation, mealGuidance (0 to 4 strings), dailyTasks (0 to 2 short general action strings), and dailyTaskReasons (one short reason for each dailyTasks item).`; }

function geminiBody(images, mealName, context, analysisLevel = "basic") { return { contents: [{ role: "user", parts: [{ text: mealPrompt(mealName, context, analysisLevel) }, ...images] }], generationConfig: { temperature: 0.2, responseMimeType: "application/json", responseSchema: { type: "OBJECT", properties: { rating: { type: "STRING", enum: ["Excellent", "Good", "Reasonable", "Needs Adjustment"] }, score: { type: "INTEGER", minimum: 0, maximum: 100 }, indicators: { type: "OBJECT", properties: { calories: { type: "NUMBER" }, protein: { type: "NUMBER" }, carbohydrates: { type: "NUMBER" }, fats: { type: "NUMBER" }, vegetables: { type: "NUMBER" }, fibre: { type: "NUMBER" }, sugar: { type: "NUMBER" }, sodium: { type: "NUMBER" }, portionBalance: { type: "NUMBER" } }, required: ["calories", "protein", "carbohydrates", "fats", "vegetables", "fibre", "sugar", "portionBalance"] }, explanation: { type: "STRING" }, mealGuidance: { type: "ARRAY", items: { type: "STRING" }, minItems: 0, maxItems: 4 }, dailyTasks: { type: "ARRAY", items: { type: "STRING" }, minItems: 0, maxItems: 2 } }, required: ["rating", "score", "indicators", "explanation", "mealGuidance", "dailyTasks"] } } }; }
