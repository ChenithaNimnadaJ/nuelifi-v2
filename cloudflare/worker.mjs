...
const DEFAULT_GEMINI_MODELS = ["gemini-2.5-flash-lite", "gemini-2.5-flash", "gemini-3.1-flash-lite", "gemini-3.5-flash-lite", "gemini-3.5-flash", "gemini-3.6-flash", "gemini-3.7-flash"];
...
function geminiModels(env, keyName) { const validated = GEMINI_MODELS_BY_KEY[keyName] || DEFAULT_GEMINI_MODELS; const configured = keyName === "GEMINI_API_KEY_2" ? env.GEMINI_KEY_2_MODELS : env.GEMINI_KEY_1_MODELS || env.GEMINI_MODEL; const requested = String([configured, env.GEMINI_FALLBACK_MODELS].filter(Boolean).join(",")).split(",").map((item) => String(item || "").trim()).filter(Boolean); const ordered = [...new Set([...validated, ...requested])]; return ordered.filter((model) => validated.includes(model)); }
...
function mealPrompt(mealName, context = {}, analysisLevel = "basic") { return `You are Neulifi, a calm food and lifestyle companion. The output is machine-read and persisted by the application. Analyse the meal called "${mealName}" from this one photo. ${contextSummary(context)} ${analysisDepth(analysisLevel)} Treat this as user-provided context only: never infer a diagnosis, prescribe treatment, recommend medication changes, or overrule a clinician. Avoid known allergens and respect dietary preferences. Nutrition values are estimates. Return only JSON. Keep mealGuidance and dailyTasks strictly separate: mealGuidance is about this photographed meal or a future similar meal and may mention ingredients, portions, nutrients, substitutions, or preparation; dailyTasks are general actions for the person’s day and must not tell them to edit, add to, remove from, swap, portion, inspect, label-check, or otherwise change the photographed meal or a specific ingredient. ${localTaskContext(context)} For dailyTasks only, return 2 to 4 short, broad actions for the person’s day. Keep them useful but non-specific: hydration, a movement break, rest or wind-down, planning, or a simple routine. Never suggest eating, cooking, changing a meal, a specific food, ingredient, nutrient, portion, recipe, or meal time. Never use an exact clock time, named exercise, medical action, medication instruction, diagnosis, or condition-specific treatment. If it is overnight, prefer rest, hydration, or planning for later rather than suggesting food or exercise now. Use cautious, non-judgmental language. Return rating, score, indicators, explanation, mealGuidance (2 to 4 strings), dailyTasks (2 to 4 short general action strings), and dailyTaskReasons (one short reason for each dailyTasks item).`; }
