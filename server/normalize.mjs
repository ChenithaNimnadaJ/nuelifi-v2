...
export function buildMealPrompt(mealName, context = {}, analysisLevel = "basic") {
  return `You are Neulifi, a calm food and lifestyle companion. Analyse the meal called "${mealName}" for a consumer wellness app. ${contextSummary(context)} ${analysisDepth(analysisLevel)} Treat this as user-provided context only: never infer a diagnosis, prescribe treatment, recommend medication changes, or overrule a clinician. Avoid known allergens and respect dietary preferences where relevant. Nutrition values are estimates. Return only JSON. Keep mealGuidance and dailyTasks separate: mealGuidance must discuss this meal or the next similar meal and must not be imported into the task list. ${localTaskContext(context)} For dailyTasks only, return 0 to 2 short, broad, non-meal actions such as hydration, a movement break, rest, planning, or a simple routine. If no action is genuinely warranted, return an empty dailyTasks array. Do not invent generic tasks for a balanced meal or repeat an existing open action. Never suggest eating, cooking, changing a meal, a specific food, ingredient, nutrient, portion, recipe, or meal time. Never use an exact clock time, named exercise, medical action, medication instruction, diagnosis, or condition-specific treatment. Use cautious, non-judgmental language. The JSON fields are rating (Excellent, Good, Reasonable, or Needs Adjustment), score (integer 0-100), indicators (numeric calories, protein, carbohydrates, fats, vegetables, fibre, sugar, sodium, portionBalance), explanation, mealGuidance (0 to 4 strings), and dailyTasks (0 to 2 short strings).`;
}

export function analysisSchema() { return { type: "OBJECT", properties: { rating: { type: "STRING", enum: ["Excellent", "Good", "Reasonable", "Needs Adjustment"] }, score: { type: "INTEGER", minimum: 0, maximum: 100 }, indicators: { type: "OBJECT", properties: { calories: { type: "NUMBER" }, protein: { type: "NUMBER" }, carbohydrates: { type: "NUMBER" }, fats: { type: "NUMBER" }, vegetables: { type: "NUMBER" }, fibre: { type: "NUMBER" }, sugar: { type: "NUMBER" }, sodium: { type: "NUMBER" }, portionBalance: { type: "NUMBER" } }, required: ["calories", "protein", "carbohydrates", "fats", "vegetables", "fibre", "sugar", "portionBalance"] }, explanation: { type: "STRING" }, mealGuidance: { type: "ARRAY", items: { type: "STRING" }, minItems: 0, maxItems: 4 }, dailyTasks: { type: "ARRAY", items: { type: "STRING" }, minItems: 0, maxItems: 2 } }, required: ["rating", "score", "indicators", "explanation", "mealGuidance", "dailyTasks"] }; }

export function normalizeMealAnalysis(input, context = {}) {
  ...
  const safeDailyTasks = completeDailyTasks(dailyTasks.length ? dailyTasks : legacyRecommendations, context);
